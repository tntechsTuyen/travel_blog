package com.travel.server.config.web;

import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;

import com.google.common.base.Predicate;

import springfox.documentation.builders.ApiInfoBuilder;
import springfox.documentation.service.ApiInfo;
import springfox.documentation.spi.DocumentationType;
import springfox.documentation.spring.web.plugins.Docket;
import springfox.documentation.swagger2.annotations.EnableSwagger2;
import static springfox.documentation.builders.PathSelectors.regex;
import static com.google.common.base.Predicates.or;

@Configuration
@EnableSwagger2
public class SwaggerConfig {

    @Bean
    public Docket postsApi() {
        return new Docket(DocumentationType.SPRING_WEB).groupName("public-api")
                .apiInfo(apiInfo())
                .select()
                .paths(postPaths())
                .build()
                .pathMapping("/");
    }

    private Predicate<String> postPaths() {
        return or(regex("/api.*"), regex("/api/login"), regex("/api/register"));
    }

    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("Exchange API")
                .description("Exchange API reference for developers")
                .termsOfServiceUrl("http://vietkosoft.com")
                .contact("vietkosoft@gmail.com").license("Exchange License")
                .licenseUrl("vietkosoft@gmail.com").version("1.0").build();
    }
}
